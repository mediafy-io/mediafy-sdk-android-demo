...
class AppLovinBanner300x250Activity : BaseAdActivity() {
    ...
    private fun createAd() {
        // 1. Create MaxAdView
        val adFormat = MaxAdFormat.MREC
        appLovinAdView = MaxAdView(APP_LOVIN_AD_UNIT_ID, adFormat, this).apply {
            // 2. Configure ad unit
            setListener(createListener())
            layoutParams = ViewGroup.LayoutParams(
                AppLovinSdkUtils.dpToPx(this@AppLovinBanner300x250Activity, WIDTH),
                AppLovinSdkUtils.dpToPx(this@AppLovinBanner300x250Activity, HEIGHT)
            )
    ...
    companion object {
        private const val APP_LOVIN_AD_UNIT_ID = "0e81f07e8ce5c636"
